...
#include "Board.h"
#include "CommonTypes.h" // Ahora PlayerColor::PLAYER_1 es Blancas, PLAYER_2 es Negras
...
GameManager::GameManager(Board& board, ConsoleView& view, InputHandler& inputHandler)
    ...
    mMoveGenerator(),
    mCurrentPlayerTurnColor(PlayerColor::PLAYER_1), // PLAYER_1 (Blancas) ahora empiezan por defecto
    mIsGameOver(false),
    mLastMove(),
...
void GameManager::ProcessPlayerTurn() {
    ...
    while (!turnActionSuccessfullyCompleted && !mIsGameOver) {
        ...
        // --- INICIO MODIFICACIÓN para CvC con comandos "salir" y "stats" ---
        ...
        if (mCurrentGameMode == GameMode::COMPUTER_VS_COMPUTER && dynamic_cast<ComputerPlayer*>(m_currentPlayerObject)) {
            feedbackMessageY = turnMessageY + 1; // Si la pausa de CvC no usó una línea extra para el prompt ">"
            // o +2 si el prompt de CvC estaba en su propia línea. Ajustar si es necesario.
            // Si el prompt ">" de CvC estuvo en turnMessageY+2, entonces feedback en turnMessageY+3
